				...
				$.ajaxSetup({
					...
					success:function(res){
						...
						$.each(res,function(idx,item) {
							...
							$('<a/>').attr({href:"#"}).html('<img src="'+item.big_img+'"/>').addClass("big_img").appendTo($big_img).appendTo($li);

							$('<a/>').attr({href:"#"}).html('<img src="'+item.small_img+'"/>').addClass("small_img").appendTo($small_img).appendTo($li);
							...
							$('<a/>').attr({href:"#"}).html(item.nane).addClass("nane_a").appendTo($nane);
